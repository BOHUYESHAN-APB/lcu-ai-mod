...
BUILTIN_SKILLS = [
    ...
    SkillManifest("core.server_command", "1.0.0", "core", "server_command", "Send an allowlisted server command.",
                   _object_schema({"family": STRING, "command": STRING}, ["family", "command"]),
                   safety_class="server_command", schedulable=False, effects=("server.command",)),
    ...
    SkillManifest("general.follow_player", "2.0.0", "general", "follow_player", "Follow a named player.",
                  ...
                  completion="outcome", schedulable=False, effects=("body.move",)),
    ...
    SkillManifest("world.inspect_block", "1.0.0", "world", "inspect_block", "Inspect one loaded block and its state properties.",
                  _object_schema({"x": BLOCK_COORD, "y": BLOCK_COORD, "z": BLOCK_COORD}, ["x", "y", "z"]),
                  schedulable=False, durable=False, effects=("world.read",)),
    SkillManifest("world.scan_crops", "1.0.0", "world", "scan_crops", "Scan nearby loaded blocks for supported crops.",
                  _object_schema({"radius": {"type": "integer", "minimum": 1, "maximum": 16}}, ["radius"]),
                  schedulable=False, durable=False, effects=("world.read",)),
    SkillManifest("world.harvest_crop_at", "2.0.0", "world", "harvest_crop_at", "Harvest and replant one observed mature vanilla crop.",
                  _object_schema({"x": BLOCK_COORD, "y": BLOCK_COORD, "z": BLOCK_COORD,
                                  "block_id": STRING, "age": INTEGER, "target_token": STRING},
                                 ["x", "y", "z", "block_id", "age", "target_token"]),
                  safety_class="resource_mutation", duration="long_running", cancellable=True,
                  completion="outcome", schedulable=False,
                  effects=("body.move", "world.break", "inventory.consume", "world.interact")),
    SkillManifest("world.break_block_at", "2.0.0", "world", "break_block_at", "Break one observed block using normal player input.",
                  _object_schema({"x": BLOCK_COORD, "y": BLOCK_COORD, "z": BLOCK_COORD,
                                  "target_token": STRING, "face": FACE},
                                 ["x", "y", "z", "target_token"]),
                  safety_class="resource_mutation", duration="long_running", cancellable=True,
                  completion="outcome", schedulable=False, effects=("body.move", "world.break")),
    SkillManifest("world.use_block_at", "2.0.0", "world", "use_block_at", "Use one observed block and verify its effect.",
                  _object_schema({"x": BLOCK_COORD, "y": BLOCK_COORD, "z": BLOCK_COORD,
                                  "target_token": STRING, "face": FACE},
                                 ["x", "y", "z", "target_token"]),
                  safety_class="resource_mutation", duration="long_running", cancellable=True,
                  completion="outcome", schedulable=False, effects=("body.move", "world.interact")),
    SkillManifest("world.place_block_at", "2.0.0", "world", "place_block_at", "Place an inventory block at an observed destination.",
                  _object_schema({"x": BLOCK_COORD, "y": BLOCK_COORD, "z": BLOCK_COORD,
                                  "target_token": STRING, "face": FACE,
                                  "place_x": BLOCK_COORD, "place_y": BLOCK_COORD, "place_z": BLOCK_COORD,
                                  "item_id": STRING},
                                 ["x", "y", "z", "target_token", "place_x", "place_y", "place_z", "item_id"]),
                  safety_class="resource_mutation", duration="long_running", cancellable=True,
                  completion="outcome", schedulable=False,
                  effects=("body.move", "world.interact", "inventory.consume")),
    ...
    SkillManifest("gui.observe", "1.0.0", "gui", "observe_gui", "Observe the current Minecraft screen on demand.",
                   _object_schema({"include_image": {"type": "boolean"}}), schedulable=False, durable=False, effects=("gui.observe",)),
    SkillManifest("gui.keybindings", "1.0.0", "gui", "get_keybindings", "List configured Minecraft key mappings.",
                   _object_schema({}), schedulable=False, durable=False, effects=("input.read",)),
    SkillManifest("gui.click", "1.0.0", "gui", "ui_click", "Click once in the current observed screen.",
                   _object_schema({"screen_revision": INTEGER, "x": NUMBER, "y": NUMBER, "button": INTEGER}, ["screen_revision", "x", "y"]),
                   safety_class="resource_mutation", schedulable=False, durable=False, effects=("gui.input",)),
    SkillManifest("gui.key_press", "1.0.0", "gui", "key_press", "Trigger one configured Minecraft key mapping.",
                   _object_schema({"screen_revision": INTEGER, "mapping_id": STRING}, ["screen_revision", "mapping_id"]),
                   safety_class="resource_mutation", schedulable=False, durable=False, effects=("input.key",)),
