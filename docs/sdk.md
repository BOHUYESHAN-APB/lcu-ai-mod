...
SDK 核心保持传输和实现中立。第三方能力通过版本化 adapter manifest 接入，
而不是把特定 STT、TTS、语音客户端、墓碑、饰品或权限模组写死在核心 API。
manifest 至少声明：

- adapter ID、语义版本和兼容的 LCU API 范围；
- 可发现 capability，例如 `voice.input.stt`、`voice.output.tts`、`inventory.curios`；
- 输入/输出 schema、权限域、数据流向、失败码和取消语义；
- 是否需要客户端、服务端或外部进程；
- 项目许可证、源码地址、署名和分发义务。

上游必须先进行能力协商，缺少 adapter 时收到明确的 `unsupported`，不能假设
所有环境都具备同一能力。LCU 不承诺无条件兼容任意实现；目标是让遵循 manifest、
安全和版本契约的实现可以独立集成。集成者必须遵守对应开源许可证，尤其不能通过
动态加载、网络边界或重新打包规避 GPL/AGPL 的适用义务。

语音 adapter 当前仅预留接口，不进入主线实现。未来由上游选择音频输入设备、
VAD/STT/说话人识别和 TTS/虚拟麦克风输出，并继续服从身份、权限、频率和审计策略。
...
- `GET /api/v2/body-requests/{request_id}` — 轮询原始身体请求的响应、进度和终态诊断
...
- `GET /api/v2/task-presets` — 查询声明式任务预设清单与可用性
- `GET /api/v2/task-presets/{id}` — 查询单个任务预设、参数 schema、示例和底层 Skill/步骤
- `POST /api/v2/task-presets/{id}/runs` — 校验预设参数，执行单 Skill run 或持久化多步骤 workflow
- `GET /api/v2/control` — 查询当前控制模式和活动租约
- `POST /api/v2/player-pairings` — operator 为指定玩家 UUID + server ID 生成受限 player token
...
- `GET /api/v2/runs` / `GET /api/v2/runs/{id}` — 查询持久化根 Task Run；workflow 详情包含步骤 run
...
原始 body request 诊断是容量为 512 条的进程内调试记录，`durable=false`；后端重启或
容量淘汰后不可查询。正式 workflow 验收应优先使用持久化 Task Run，raw command 仅用于
确定性原语和问题定位。

## PowerShell Headed Test

仓库内 `scripts/headed-test.ps1` 默认只读取状态，不会控制身体：

```powershell
pwsh -File scripts/headed-test.ps1 -Mode Status
pwsh -File scripts/headed-test.ps1 -Mode Discover
```

显式动作模式会先验证 body 在线、armed 且 observation 未 stale。`Raw` 轮询
`body-requests` 终态；`Skill`/`Preset` 自动申请 external lease、定期 heartbeat、轮询
run 并在 finally 中释放租约：

```powershell
pwsh -File scripts/headed-test.ps1 -Mode Raw -Command get_state
pwsh -File scripts/headed-test.ps1 -Mode Skill -SkillId core.move_to `
  -InputJson '{"x":1,"y":64,"z":2}' -OutputPath backend/.local/move-test.json
pwsh -File scripts/headed-test.ps1 -Mode Preset -PresetId survival.eat
```

诊断导出会隐藏 fencing token。脚本退出码 `2` 表示终态为 failed、cancelled 或 unknown。
远程 `BaseUrl` 必须使用 HTTPS；退出码 `3` 表示租约释放失败，`4` 表示诊断导出失败。
...
with LCUClient("http://127.0.0.1:8080", api_token="token") as client:
    ...
    try:
        presets = client.list_task_presets("crafting")
        run = client.run_task_preset(
            "craft.iron_pickaxe",
            {},
            lease_id=lease["id"],
...
当前测试与本地开发环境中的 `backend/.local/agent_state.db` 使用迁移化 SQLite 保存 Skill 元数据、控制租约、Task Run、workflow 父子关系、日程和领域事件；可执行 Skill 代码不存入数据库。SQLite 不得用于生产部署，生产环境必须使用 PostgreSQL，且不得静默回退。使用 `LCU_ENV` 与 `LCU_STORAGE_BACKEND` 显式声明环境和存储；PostgreSQL 适配器完成前，生产模式会拒绝启动。V2 Skill run 返回稳定 UUID `id`；派发后 wire `request_id` 使用同一个 UUID，排队时为 `null`。断线时已派发但没有终态的 run 会标记为 `unknown`，不会静默重放。
...
任务预设适合控制台和上游自动化直接触发常用工作流，例如制作铁镐、收集原木、移动到坐标、进食，以及“收集木材后制作箱子”。原始 Skill 运行入口保留，适合高级调试、实验性组合和低层能力验证。多步骤 workflow preset 当前不能创建 schedule；schedule 仍只接受单个可调度 Skill。
...
`send_command` 返回的 request ID 可传给 `get_body_request`（浏览器为 `getBodyRequest`）。诊断记录包含命令、已限长且按敏感字段名脱敏的参数、请求方、状态、进度、详情、身体响应/终态 payload 和时间戳。原始请求诊断是最多保留 512 条的进程内调试数据，响应中明确返回 `durable=false`；重启或容量淘汰后不可查询。断线或 30 分钟未收到所需终态的请求标记为 `unknown`。
